...
@Slf4j
@Service("IOService")
public class PamiIOServiceRobot implements PamiIOService, InitializingBean, DisposableBean {
...
//    private GpioPinDigitalInput inIrq6;
    ...
    public void destroy() throws Exception {
        ...
        try {
            ...
            log.warn("Problème de shutdown du {} : {}", PamiConstantesI2C.PCF1_DEVICE_NAME, e.getMessage());
        }
        try {
            if (pcf2 != null) {
                pcf2.shutdown();
            ...
            log.warn("Problème de shutdown du {} : {}", PamiConstantesI2C.PCF2_DEVICE_NAME, e.getMessage());
        }
        try {
            if (gpio != null) {
                gpio.shutdown();
...
//        inIrq6 = gpio.provisionDigitalInputPin(RaspiPin.GPIO_06);
        ...
        // Config PCF8574 //
        // -------------- //
        pcfAlim = new PCF8574GpioProvider(bus, PamiConstantesI2C.PCF_ALIM_ADDRESS, true);
        pcf1 = new PCF8574GpioProvider(bus, PamiConstantesI2C.PCF1_ADDRESS, true);
        pcf2 = new PCF8574GpioProvider(bus, PamiConstantesI2C.PCF2_ADDRESS, true);
    ...
    public void refreshAllIO() {
        try {
            ...
            log.error("Erreur lecture " + PamiConstantesI2C.PCF1_DEVICE_NAME + " : " + e.getMessage(), e);
        }
        ...
        try {
            ...
            log.error("Erreur lecture " + PamiConstantesI2C.PCF2_DEVICE_NAME + " : " + e.getMessage(), e);
        }
