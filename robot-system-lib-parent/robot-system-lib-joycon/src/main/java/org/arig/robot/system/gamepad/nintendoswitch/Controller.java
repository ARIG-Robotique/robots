...
public abstract class Controller {
    ...
    protected abstract ControllerEventListener eventListener();
    protected abstract void processData(byte[] data);
    protected abstract Map<ControllerButton, Boolean> inputs();
    ...
    public boolean close() {
        ...
        try {
            ...
            log.warn("Erreur lors de la fermeture de la connexion avec le controller {} - {}", controllerId, name);
            isClosed = false;
        }
        return isClosed;
    }
    ...
    public void open() {
        ...
        for (HidDeviceInfo info : PureJavaHidApi.enumerateDevices()) {
            if ((info.getVendorId() == NintendoSwitchHID.VENDOR_ID) && (info.getProductId() == controllerId)) {
        ...
        if (hidDeviceInfo == null) {
            log.info("Aucun device correspondant trouvé pour {} - {}", controllerId, name);
            return;
        }
        ...
        try {
            ...
            device.setInputReportListener(new ControllerInputListener(this));
